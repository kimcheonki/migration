...
사용자 친화적인 대화형 모드를 사용하면 복잡한 명령행 인자 없이 쉽게 마이그레이션할 수 있습니다. 설정파일을 자동으로 감지하고 GUI 파일 선택 대화상자로 직관적으로 선택할 수 있습니다.
...
1. **GUI 파일 선택**: 파일 선택 대화상자에서 설정파일 선택
2. **서버 자동 감지**: 감지된 서버 확인 또는 수동 선택
3. **버전 입력**: AS-IS 및 TO-BE 버전 입력
4. **GUI 디렉토리 선택**: 출력 디렉토리 선택 대화상자
5. **레포트 형식 선택**: HTML/Excel/둘 다 선택
6. **마이그레이션 실행**: 확인 후 실행
